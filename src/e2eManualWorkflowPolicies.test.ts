...
describe('manual e2e workflow policies', () => {
  ...
  test('the reusable workflow keeps credentials step-scoped, accepts typed candidate identity inputs, handles stale manual and automatic callers safely, and tears down by exact app ID', () => {
    ...
    expect(reusableWorkflow).toContain("import { generateHealthValue, HEALTH_VALUE_ENV_NAME } from './.candidate-source/src/e2e/health-value.ts'")
    expect(reusableWorkflow).toContain('name: Create second healthy fixture commit')
    expect(reusableWorkflow).toContain("import { createHealthyFixtureCommit } from './.candidate-source/src/e2e/git-fixture.ts'")
    expect(reusableWorkflow).toContain('name: Deploy healthy fixture commit with quiet env checks')
    ...
    expect(reusableWorkflow).toContain('E2E_HEALTH_VALUE=${{ steps.health-value.outputs.value }}')
    ...
    expect(reusableWorkflow).toContain('name: Observe healthy fixture env deployment')
    ...
    expect(reusableWorkflow).toContain('lookupEnvironmentValue: controller.getEnvironmentValue')
    expect(reusableWorkflow).toContain('name: Verify quiet deployment log markers')
    expect(reusableWorkflow).toContain('name: Capture same-commit baseline state')
    expect(reusableWorkflow).toContain('name: Deploy same commit with default error policy')
    expect(reusableWorkflow).toContain('continue-on-error: true')
    expect(reusableWorkflow).toContain('name: Assert same-commit error outcome and activity')
    expect(reusableWorkflow).toContain('Remote HEAD has the same commit as the one to push')
    expect(reusableWorkflow).toContain('name: Deploy same commit with ignore policy')
    ...
    expect(reusableWorkflow).toContain('name: Deploy same commit with restart policy')
    expect(reusableWorkflow).toContain('sameCommitPolicy: restart')
    expect(reusableWorkflow).toContain('name: Observe same-commit restart state')
    expect(reusableWorkflow).toContain('Expected sameCommitPolicy: restart to report a new instance ID')
    expect(reusableWorkflow).toContain('name: Deploy same commit with rebuild policy')
    expect(reusableWorkflow).toContain('sameCommitPolicy: rebuild')
    expect(reusableWorkflow).toContain('name: Observe same-commit rebuild state')
    expect(reusableWorkflow).toContain('Expected sameCommitPolicy: rebuild to report a new instance ID')
    ...
    expect(reusableWorkflow).toContain('Expected the default same-commit policy to fail')
  ...
  test('the reusable workflow rejects divergent history without force, then deploys the same commit with force by app ID', () => {
    ...
    expect(reusableWorkflow).toContain("import { createDivergentFixtureCommit } from './.candidate-source/src/e2e/git-fixture.ts'")
    expect(reusableWorkflow).toContain('name: Deploy divergent fixture commit without force')
    expect(reusableWorkflow).toContain('continue-on-error: true')
    expect(reusableWorkflow).toContain('name: Assert divergent rejection outcome and preserved production')
    expect(reusableWorkflow).toContain('Expected divergent deployment without force to fail')
    expect(reusableWorkflow).toContain('Expected divergent deployment without force log to mention a non-fast-forward rejection')
    expect(reusableWorkflow).toContain('name: Deploy divergent fixture commit with force')
    expect(reusableWorkflow).toContain('force: true')
    expect(reusableWorkflow).toContain('name: Observe forced divergent deployment')
    expect(reusableWorkflow).toContain("import { confirmRejectedDeploymentPreservesLiveApp, waitForNewHealthyDeployment } from './.candidate-source/src/e2e/deployment-observer.ts'")
    expect(reusableWorkflow).toContain('ACTION_OUTCOME: ${{ steps.divergent-no-force.outcome }}')
    expect(reusableWorkflow).toContain('ACTION_OUTCOME: ${{ steps.divergent-force.outcome }}')

    const recoveryIndex = reusableWorkflow.indexOf('name: Observe recovery fixture deployment')
    const baselineIndex = reusableWorkflow.indexOf('name: Capture force baseline state')
    const divergentCommitIndex = reusableWorkflow.indexOf('name: Create divergent fixture commit')
    const noForceIndex = reusableWorkflow.indexOf('name: Deploy divergent fixture commit without force')
    const assertNoForceIndex = reusableWorkflow.indexOf('name: Assert divergent rejection outcome and preserved production')
    const forceIndex = reusableWorkflow.indexOf('name: Deploy divergent fixture commit with force')
  ...
  test('the reusable workflow preserves production through build and startup failures, then recovers in order', () => {
    ...
    expect(reusableWorkflow).toContain('continue-on-error: true')
    expect(reusableWorkflow).toContain('CC_POST_BUILD_HOOK=node scripts/post-build-hook.mjs')
    expect(reusableWorkflow).toContain('E2E_SCENARIO=build-failure')
    expect(reusableWorkflow).toContain('name: Assert build-failure outcome and preserved production')
    expect(reusableWorkflow).toContain('Expected build-failure deployment to fail')
    expect(reusableWorkflow).toContain("expectedCommitID: previousState.commitId")
    expect(reusableWorkflow).toContain("expectedDeploymentID: previousState.deploymentId")
    expect(reusableWorkflow).toContain('Expected build-failure deployment to preserve the prior healthy instance ID')
    expect(reusableWorkflow).toContain('name: Create startup-failure fixture commit')
    expect(reusableWorkflow).toContain('name: Deploy startup-failure fixture commit')
    expect(reusableWorkflow).toContain('E2E_SCENARIO=startup-failure')
    expect(reusableWorkflow).toContain('name: Assert startup-failure outcome and preserved production')
    expect(reusableWorkflow).toContain('Expected startup-failure deployment to fail')
    expect(reusableWorkflow).toContain('Expected startup-failure deployment to preserve the prior healthy instance ID')
    ...
    expect(reusableWorkflow).toContain('name: Observe recovery fixture deployment')
    expect(reusableWorkflow).toContain('expectedScenario: \'healthy\'')
    expect(reusableWorkflow).toContain('EXPECTED_COMMIT_ID: ${{ steps.recovery-commit.outputs.commit }}')

    const rebuildIndex = reusableWorkflow.indexOf('name: Observe same-commit rebuild state')
    const buildFailureIndex = reusableWorkflow.indexOf('name: Create build-failure fixture commit')
    const startupFailureIndex = reusableWorkflow.indexOf('name: Create startup-failure fixture commit')
    const recoveryIndex = reusableWorkflow.indexOf('name: Create recovery fixture commit')
